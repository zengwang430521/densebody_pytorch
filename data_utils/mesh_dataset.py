...
windows_root = 'D:/data'
linux_root = '/home/wzeng/mydata/h3.6m'
...
class MeshDataset(Dataset):
    def __init__(self, data_root=data_root, image_folder='images_washed',
        ...
        self.im_root = data_root + '/' + image_folder
        if not os.path.isdir(self.im_root):
            raise (FileNotFoundError('{} dataset not found, '.format(image_folder) +
        ...
        if phase == 'in_the_wild':
            ...
            self.mesh_root = data_root + '/' + image_folder.replace('images', 'meshes')
            if not os.path.isdir(self.mesh_root):
                raise (FileNotFoundError('{} not found, '.format(self.mesh_root) +
                                         'please generate mehses first'))
            ...
            for k in tmp.keys():
                ...
                if k == 'imagename':
                    ...
                    self.mesh_names = [self.mesh_root + s.replace('jpg', 'obj') for s in data]
                    self.itemlist.append('im_names')
                    self.itemlist.append('mesh_names')
                else:
                    setattr(self, k, data)
